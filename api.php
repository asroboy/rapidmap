...
class PHP_CRUD_API {
	...
	protected function getParameters($settings) {
		...
		$distinct  = $this->parseGetParameter($get, 'distinct', 'a-zA-Z0-9\-_,.*');
		...
		if($distinct != null || $distinct != ""){
			$fields = $this->limitFields($allFields,$distinct,$exclude,$select);
		}else{
		...
		return compact('action','database','tables','key','page','filters','fields','orderings','transform','inputs','collect','select','before','after','distinct');
	}
	...
	protected function listCommand($parameters) {
		...
		if ($transform) {
			...
			json_encode(self::php_crud_api_transform($data));
		}
		return false;
	}
